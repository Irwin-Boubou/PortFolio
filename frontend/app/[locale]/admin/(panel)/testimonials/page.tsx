...
interface Row {
  id: string; name: { en: string }; role: { en: string }; company: string;
  rating: number; featured: boolean; published: boolean; order: number;
}
...
export default function AdminTestimonialsPage() {
  ...
  const toggleFeatured = useMutation({
    mutationFn: (id: string) => api.patch(`/testimonials/${id}/featured`),
    onSuccess: invalidate,
  });
  const reorder = useMutation({
    mutationFn: (order: { id: string; order: number }[]) => api.patch('/testimonials/reorder', { order }),
    onSuccess: invalidate,
  });
  ...
  return (
    <div>
      <div className="mb-8 flex items-center justify-between">
        ...
        <Link href="/admin/testimonials/new" className="rounded-lg bg-[#6C63FF] px-4 py-2 text-sm font-medium text-white hover:bg-[#5a51f0]">
          + New testimonial
        </Link>
      </div>
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={r.id} className="border-b border-gray-100 last:border-0">
                ...
                <td className="px-5 py-3 text-gray-500">{r.role.en} · {r.company}</td>
                <td className="px-5 py-3 text-gray-500">{r.rating}★</td>
                ...
                <td className="px-5 py-3">
                  <button onClick={() => toggleFeatured.mutate(r.id)}
                          className={r.featured ? 'text-yellow-500' : 'text-gray-300 hover:text-gray-400'}>★</button>
                </td>
                <td className="px-5 py-3">
                ...
                <td className="px-5 py-3 text-right">
                  <Link href={`/admin/testimonials/${r.id}/edit`} className="mr-4 text-[#6C63FF] hover:underline">Edit</Link>
                  <button onClick={() => { if (confirm('Delete this testimonial? This cannot be undone.')) del.mutate(r.id); }}

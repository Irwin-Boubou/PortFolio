...
interface Row {
  id: string; title: { en: string }; issuer: { en: string }; date: string; published: boolean; order: number;
}
...
export default function AdminAwardsPage() {
  ...
  const del = useMutation({
    mutationFn: (id: string) => api.delete(`/awards/${id}`),
    onSuccess: () => { toast.success('Deleted'); invalidate(); },
  });
  const reorder = useMutation({
    mutationFn: (order: { id: string; order: number }[]) => api.patch('/awards/reorder', { order }),
  ...
  return (
    <div>
      <div className="mb-8 flex items-center justify-between">
        <h1 className="font-display text-2xl font-bold">Awards</h1>
        <Link href="/admin/awards/new" className="rounded-lg bg-[#6C63FF] px-4 py-2 text-sm font-medium text-white hover:bg-[#5a51f0]">
          + New award
        </Link>
      </div>
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={r.id} className="border-b border-gray-100 last:border-0">
                <td className="px-5 py-3 font-medium">{r.title.en}</td>
                <td className="px-5 py-3 text-gray-500">{r.issuer.en}</td>
                ...
                <td className="px-5 py-3 text-right">
                  <Link href={`/admin/awards/${r.id}/edit`} className="mr-4 text-[#6C63FF] hover:underline">Edit</Link>
                  <button onClick={() => { if (confirm('Delete this award? This cannot be undone.')) del.mutate(r.id); }}

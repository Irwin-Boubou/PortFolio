...
interface Props { title?: string; subtitle?: string; primary?: string; secondary?: string }

export function ContactCTA({ title, subtitle, primary, secondary }: Props = {}) {
  ...
  return (
    <section className="relative overflow-hidden py-24">
      ...
      <div className="relative mx-auto max-w-content px-6 text-center">
        ...
        <p className="mx-auto mt-3 max-w-xl text-muted">{subtitle || t('subtitle')}</p>
        <div className="mt-8 flex flex-wrap justify-center gap-4">
          <Button href="/contact">{primary || t('primary')}</Button>
          <Button href="/work/development" variant="outline">{secondary || t('secondary')}</Button>
        </div>
      </div>
    </section>
  );
